...
func set(s, path, raw string, stringify, del, optimistic, place bool) ([]byte, error) {
	if path == "" {
		...
		return kbstring.String2Bytes(raw), nil
	}
		...
		if res.Exists() && res.Index > 0 {
			...
			if place && sz <= len(s) {
				if !stringify || !mustMarshalString(raw) {
					...
					if stringify {
						jbytes[res.Index] = '"'
						copy(jbytes[res.Index+1:], kbstring.String2Bytes(raw))
						jbytes[res.Index+1+len(raw)] = '"'
						copy(jbytes[res.Index+1+len(raw)+1:],
							jbytes[res.Index+len(res.Raw):])
					} else {
						copy(jbytes[res.Index:], kbstring.String2Bytes(raw))
...
func SetOptions(json, path string, value interface{},
	...
	jsonb := kbstring.String2Bytes(json)
	res, err := SetBytesOptions(jsonb, path, value, opts)
	return kbstring.Bytes2String(res), err
}
...
func SetBytesOptions(json []byte, path string, value interface{},
	...
	jstr := kbstring.Bytes2String(json)
	var res []byte
	var err error
	switch v := value.(type) {
	default:
...
func SetRawBytesOptions(json []byte, path string, value []byte,
	opts *Options) ([]byte, error) {
	jstr := kbstring.Bytes2String(json)
	vstr := kbstring.Bytes2String(value)
	var optimistic, inplace bool
	if opts != nil {
		optimistic = opts.Optimistic
		inplace = opts.ReplaceInPlace
