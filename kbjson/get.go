...
func ParseBytes(json []byte) Res {
	return Parse(kbstring.Bytes2String(json))
}
...
func parseQuery(query string) (
	...
	if j > 0 {
		path = kbstring.TrimSpace(query[2:j])
		value = kbstring.TrimSpace(query[j:i])
		remain = query[i+1:]
		var opsz int
		switch {
		case len(value) == 1:
		...
		op = value[:opsz]
		value = kbstring.TrimSpace(value[opsz:])
	} else {
		path = kbstring.TrimSpace(query[2:i])
		remain = query[i+1:]
	}
			...
			if i < len(path) {
				...
				for ; i < len(path); i++ {
					if path[i] == '\\' {
						...
						r.part = kbstring.Bytes2String(epart)
						if ModifiersState() &&
...
func parseObject(c *parseContext, i int, path string) (int, bool) {
	...
	for i < len(c.json) {
		...
		if rp.wild {
			if kesc {
				pmatch = kbstring.Match(unescape(key), rp.part)
			} else {
				pmatch = kbstring.Match(key, rp.part)
			}
		} else {
			if kesc {
...
func queryMatches(rp *arrayPathResult, value Res) bool {
	...
	case String:
		...
		case ">=":
			return value.Str >= rpv
		case "%":
			return kbstring.Match(value.Str, rpv)
		case "!%":
			return !kbstring.Match(value.Str, rpv)
		}
	case Number:
			...
			case ']':
				if rp.arrch && rp.part == "#" {
					if rp.alogok {
						...
						for j, k := 0, 0; j < len(alog); j++ {
							...
							if ok {
								...
								if res.Exists() {
									...
									jsons = append(jsons, kbstring.String2Bytes(raw)...)
									k++
								}
							}
						}
						jsons = append(jsons, ']')
						c.value.Type = JSON
						c.value.Raw = kbstring.Bytes2String(jsons)
						return i + 1, true
					}
					if rp.alogok {
						break
				...
				if len(multires) > 0 && !c.value.Exists() {
					c.value = Res{
						Raw:  kbstring.Bytes2String(append(multires, ']')),
						Type: JSON,
					}
				}
				return i + 1, false
...
func appendJSONString(dst []byte, s string) []byte {
	for i := 0; i < len(s); i++ {
		if s[i] < ' ' || s[i] == '\\' || s[i] == '"' || s[i] > 126 {
			d, _ := json.Marshal(s)
			return append(dst, kbstring.Bytes2String(d)...)
		}
	}
	dst = append(dst, '"')
	dst = append(dst, s...)
	...
	if len(path) > 1 {
		...
		if path[0] == '[' || path[0] == '{' {
			...
			if ok && len(path) == 0 || (path[0] == '|' || path[0] == '.') {
				...
				b = append(b, kind+2)
				var res Res
				res.Raw = kbstring.Bytes2String(b)
				res.Type = JSON
				if len(path) > 0 {
					res = res.Get(path[1:])
				}
...
func GetBytes(json []byte, path string) Res {
	return Get(kbstring.Bytes2String(json), path)
}
...
func unescape(json string) string {
	...
	for i := 0; i < len(json); i++ {
		switch {
		case json[i] < ' ':
			return kbstring.Bytes2String(str)
		case json[i] == '\\':
			i++
			if i >= len(json) {
				return kbstring.Bytes2String(str)
...
func Valid(json string) (ok bool) {
	_, ok = validPayload(kbstring.String2Bytes(json), 0)
	return
}
...
func modifierPretty(json, arg string) string {
	if len(arg) > 0 {
		...
		return kbstring.Bytes2String(FormatOptions(kbstring.String2Bytes(json), &opts))
	}
	return kbstring.Bytes2String(Format(kbstring.String2Bytes(json)))
}

func modifierUgly(json, _ string) string {
	return kbstring.Bytes2String(Ugly(kbstring.String2Bytes(json)))
}
...
func modifierReverse(json, _ string) string {
	...
	if res.IsArray() {
		...
		out = append(out, ']')
		return kbstring.Bytes2String(out)
	}
	if res.IsObject() {
		var keyValues []Res
